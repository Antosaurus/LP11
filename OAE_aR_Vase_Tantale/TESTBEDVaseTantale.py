# -*- coding: utf-8 -*-
"""
Created on Fri Apr 15 17:22:00 2022
Domaine : Mécanique
Sous-Domaine : Oscillateurs
Chapitre : Oscillateur de relaxation
Appellation : Vase de Tantale
@author: Antonin
"""
#-Clean Workspace
...
eta = np.linspace(0.010,0.9999999*etalim,Np)#q.epsilon = D/D0
# 
# --- Evolution de la durée de la vidange w_V(r) ---
# w_v = t_v / tau
# w_v = 2*epsilon*(1-etalim+eta*ln((1-eta)/(etalim-eta)))
w_v = 2*eps*(1-etalim + eta*np.log((1-eta)/(etalim-eta)))
# --- Evolution de la durée de remplissage w_R(r)---
...
plt.xlabel('$D/D_0$')
plt.ylabel('$T/\\tau$')
plt.title(('Evolution de la période des oscillations avec le débit \n $\\epsilon=$ %0.2f'%eps + ' et ' + '$D_1/D_0=$%0.2f'%etalim))
""" 
...
# 
vecr = eta #np.linspace(0.005,etalim,Np)
# 
